...
function getStorageSync(params) {
  if (typeof wx !== 'undefined') {
    ...
    let res = localStorage.getItem(params)
...
class _hooks {
  constructor(props={}) {
    ...
    if (this.storage) {
      let oldData = getStorageSync(this.namespace)
      let oldExpireData = getStorageSync(this.expireDataKey)
  ...
  syncData(key, type){
    ...
    if (this.storage) {
      ...
      this.syncTimmer = setTimeout(() => {
        setStorageSync(this.namespace, this.storeData)
        setStorageSync(this.expireDataKey, this.expireData)
      }, 500);
    }
  }
  ...
  clear(){
    this.destory()
    if (this.storage) {
      removeStorage({key: this.namespace})
      removeStorage({key: this.expireDataKey})
    }
    // wx.clearStorageSync()
  }
